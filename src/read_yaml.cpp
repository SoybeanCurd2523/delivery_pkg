...
void YAML_CONFIG_READER::get_XY_Coordinates_From_yaml(char room[]){
  ...
  try {
      ...
      YAML::Node room_number = doc[room];
      x_distance = room_number["x"].as<double>();
      y_distance = room_number["y"].as<double>();
    
      path_x.insert({room, x_distance});
      path_y.insert({room, y_distance});
...
double YAML_CONFIG_READER::get_x(char room[]){ 
    return path_x[room];
}

double YAML_CONFIG_READER::get_y(char room[]){ 
    return path_y[room];
}
